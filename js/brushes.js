(() => {
  ...
  const BRUSHES = {
    ...
    'Карандаш (мягкий)': (ctx, x, y, r, color, op) => {
      ...
      for(let i=0;i<r;i++){ctx.fillRect(x+rand(r*0.3),y+rand(r*0.3),1,1);} ctx.restore();
    },
    ...
    'Сухая кисть': (ctx, x, y, r, color, op) => {
      ...
      for(let i=0;i<r*1.5;i++){
        ctx.fillRect(x+rand(r),y+rand(r*0.4),1,1);
      }
      ctx.restore();
    },
    ...
    'Акварель (расширенная)': (ctx, x, y, r, color, op) => {
      ...
      ctx.globalAlpha=op*0.15; for(let i=0;i<r*2;i++){ctx.fillRect(x+rand(r),y+rand(r),1,1);} ctx.restore();
    },
    'Холст': (ctx, x, y, r, color, op) => { ctx.save(); ctx.globalAlpha=op*0.3; ctx.strokeStyle=color;
    ...
    'Бумага (акварельная)': (ctx, x, y, r, color, op) => { ctx.save(); ctx.globalAlpha=op*0.2; ctx.fillStyle=color;
      for(let i=0;i<r*2;i++){ctx.fillRect(x+rand(r),y+rand(r),1,1);} ctx.restore();
    },
    'Песок': (ctx, x, y, r, color, op) => { ctx.save(); ctx.globalAlpha=op*0.5; ctx.fillStyle=color;
      for(let i=0;i<r*3;i++){ctx.beginPath(); ctx.arc(x+rand(r),y+rand(r),Math.random()*1.5,0,Math.PI*2); ctx.fill();}
      ctx.restore();
    },
    'Мрамор': (ctx, x, y, r, color, op) => { ctx.save(); ctx.globalAlpha=op*0.4; ctx.strokeStyle=color; ctx.lineWidth=1;
    ...
    'Камень': (ctx, x, y, r, color, op) => {
      ...
      for (let i=0;i<r*2;i++){ctx.beginPath(); ctx.arc(x+rand(r), y+rand(r), Math.random()*2, 0, Math.PI*2); ctx.fill();}
      ctx.restore();
    },
    ...
    'Гуашь': (ctx, x, y, r, color, op) => {
      ...
      ctx.beginPath(); ctx.arc(x,y,r,0,Math.PI*2); ctx.fill();
      ctx.globalAlpha=op*0.2; for(let i=0;i<r;i++){ctx.fillRect(x+rand(r),y+rand(r),1,1);} ctx.restore();
    },
